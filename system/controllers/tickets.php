...
switch ($action){
    ...
    case 'create':
        ...
        $ui->assign('xfooter',
            Asset::js(array('modal','dropzone/dropzone','s2/js/select2.min','s2/js/i18n/'.lan()))
        );
 ...
 ');
    ...
    case 'json_list':
        ...
        if($company != '') {
            ...
            $contacts_under_companies = Contact::where('company', 'like', '%' . $company . '%')->get();
        ...
        $contacts = Contact::select(['id','account','company'])->get()->keyBy('id')->toArray();
        ...
        foreach ($x as $xs){
            ...
            $img = '<span class="clx-avatar ib_bg_'.$css_bg.'">'.$first_name_letter.$last_name_letter.'</span>';
            ...
            if(isset($contacts[$xs['userid']]) && $contacts[$xs['userid']]['company'] != '')
            {
                $company .= $contacts[$xs['userid']]['company'];
            }
            ...
            $records["data"][] = array(
                0 => $i,
                1 => '<a href="'.U.'contacts/view/'.$xs['userid'].'">'.$img.'</a>',
                2 => $xs['subject'].' <br>'.$xs['tid'],
                3 => $account,
                4 => $company,
                5 => $staff_name,
                6 =>  '
    ...
    case 'update_status':
        ...
        if($d){
	        ...
	        _log('Ticket - <a href="'.U.'tickets/admin/view/'.$d->id.'">'.$d->tid.'</a> updated By- '.$user->fullname.' Value: '.$value,'Ticket',$user->id);
...
//        }
        ...
        if($d){
	        ...
	        _log('Ticket assigned By- '.$user->fullname.' Assigned To: '.$staff->fullname,'Ticket',$user->id);
    ...
    case 'tasks_list':
        ...
        foreach ($tasks as $task){
            $li .= '<li class="task_item'.(($task['status'] == 'Completed') ? ' completed':'').'" id="t_tasks_'.$task['id'].'">
                                <input class="custom-checkbox task-checkbox" id="s_tasks_'.$task['id'].'" type="checkbox" value="" name="" '.(($task['status'] == 'Completed') ? ' checked':'').' class="i-checks"/>
