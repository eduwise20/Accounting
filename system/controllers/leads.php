...
switch ($action) {
    ...
    case 'json_list':
        ...
        foreach($x as $xs) {
            ...
            $records["data"][] = array(
                ...
                '
                <div class="btn-group">
                <a href="#" class="btn btn-primary btn-xs cview" id="vid' . $xs['id'] . '"><i class="fal fa-search"></i> </a>
                <a href="#" class="btn btn-warning btn-xs cedit" id="eid' . $xs['id'] . '"><i class="fal fa-pencil"></i> </a>
                <a href="#" class="btn btn-danger btn-xs cdelete" id="did' . $xs['id'] . '"><i class="fal fa-trash-alt"></i> </a>
</div>
                ',
            );
        }
    ...
    case 'web-to-lead':
        ...
        $lead_form_ids = $lead_forms->keyBy('id')->keys()->all();
    ...
    case 'form-embed':
        ...
        if($id){
            ...
            if($lead_form)
            {
                $embed_code = '<iframe width="100%" height="800"
                  src="'.U.'client/form/'.$lead_form->uuid.'/embed/" 
                  frameborder="0" 
                  allowfullscreen>
    ...
    case 'save-form-data':
        ...
        if($id)
        {
            ...
            if($lead_form && !empty($_POST['form_data']))
            {
                $lead_form->form_data = $_POST['form_data'];
                $lead_form->save();
            }
