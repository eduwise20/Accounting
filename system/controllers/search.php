...
if($d){
 ...
 '.$_L['Image'].'
    ...
    foreach ($d as $ds){
        ...
        if($ds['image'] == ''){
            $img = '<img src="'.APP_URL.'/ui/lib/img/item_placeholder.png">';
        }
        else{
      ...
      //  $price = number_format($ds['sales_price'],2,$config['dec_point'],$config['thousands_sep']);
        ...
        if($can_edit)
        {
            $edit_button = '<a href="#" class="btn btn-primary cedit" id="e'.$ds['id'].'"><i class="fal fa-pencil"></i> '.$_L['Edit'].' </a>';
        }

        if($can_delete)
        {
            $delete_button = '<a href="#" class="btn btn-danger cdelete" id="pid'.$ds['id'].'"><i class="fal fa-trash-alt"></i> '.$_L['Delete'].' </a>';
        }
 ...
 '.$img.'
                ...
                '.$edit_button.'
                    '.$delete_button.'
                    <a href="'.U.'inventory/barcode/'.$ds['id'].'" target="_blank" class="btn btn-dark"><i class="fal fa-barcode"></i> '.$_L['Barcode'].' </a>
</div>
