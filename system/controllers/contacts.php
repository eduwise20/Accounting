...
_L[\'New Company\'] = \''.$_L['New Company'].'\';
 ');
    ...
    case 'activity':
        ...
        $d = ORM::for_table('crm_accounts')->find_one($cid);
        if($d){
            $ac = ORM::for_table('sys_activity')->where('cid',$cid)->limit(30)->order_by_desc('id')->find_many();
            $ui->assign('ac',$ac);
    ...
    case 'invoices':
        ...
        if($d){
            ...
            view('ajax.contact-invoices',[
                'total_invoice_amount' => $invoices_summary['total_invoiced_amount'],
    ...
    case 'activity-delete':
        ...
        $d->delete();
        $cid = $routes['2'];
        r2(U.$myCtrl.'/view/'.$cid.'/activity','s',$_L['Deleted Successfully']);
        break;
...
";
    ...
    case 'json-single-contact':
        ...
        if($contact)
        {
            ...
            $address_full = getContactFormattedAddress($config,$contact,false);
...
//            }
            ...
            try{
                ...
                $client->request('GET', $data['logo_url'], ['sink' => 'storage/companies/'.$logo_path]);
            } catch (\Exception $e)
            {
    ...
    case 'company_memo':
        ...
        if($d){
            echo '<textarea class="form-control" id="v_memo" name="v_memo" rows="6">'.$d->notes.'</textarea> <button type="button" id="memo_update" class="btn btn-primary btn-block mt-sm act_memo_update mt-3">'.$_L['Save'].'</button>';
        }
    ...
    case 'company_summary':
        ...
        if($d){
            ...
            echo '<p>
                        ...
                        <a href="#" class="btn btn-primary cedit" id="me'.$d->id.'">'.$_L['Edit'].'</a>
                        
                        
                        <hr>
                        
                        <a href="#" class="btn btn-primary li_memo">'.$_L['Memo'].'</a>
        ...
        if($d){
            ...
            <td>
                ...
                if($dt == ''){
                    ...
                    $tds = '<tr><td colspan="8">'.$_L['No Data Available'].'</td> </tr>';
        ...
        if($d){
            ...
            <td>'.$order['status'].'</td>
                ...
                if($dt == ''){
                    ...
                    $tds = '<tr><td colspan="5">'.$_L['No Data Available'].'</td> </tr>';
