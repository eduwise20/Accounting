            ...
            foreach($description as $item) {
             ...
             //   $samount = createFromCurrency($samount,$currency_id);
              ...
              //  $sqty = Finance::amount_fix($sqty);
                ...
                if($discount_type == 'p')
                {
                    $actual_line_discount = $line_total*($line_discount/100);
                }

                $line_total = $line_total-$actual_line_discount;
...
//                    }
    ...
    <label for="amount">' . $_L['Amount'] . ' ['.$currency_iso_code.']</label>
...
<div class="form-group">
    <label for="subject">' . $_L['Account'] . '</label>
...
<div class="form-group">
  ...
  <div class="form-group">
    <label for="subject" class="mb-3">' . $_L['Message Body'] . '</label>
...
<div class="form-group">
    ...
    <div class="custom-checkbox c-checkbox">
                          <label>
                            <input type="checkbox" name="attach_pdf" id="attach_pdf" value="Yes" checked> '.$_L['Invoice'].'_' . $in . '.pdf
                          </label>
                        </div>
  </div>
        ...
        if ($msg == '') {
           ...
           // Notify_Email::_send($toname, $email, $subject, $message, $cid, $iid, $cc, $bcc, $attachment_path, $attachment_file);
            ...
            if($email_config)
            {
                ...
                {
                    $transport = (new Swift_SmtpTransport($email_config->host, $email_config->port,$email_config->secure))
